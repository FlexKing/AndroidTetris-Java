    ...
    private synchronized void next() {
        ...
        if (isNextMerged()) {
                    ...
                    for (int i = 0; i <= y; i++) {
                        for (int j = 0; j < PLAYING_AREA_WIDTH; j++) {
                            ...
                            if (point.type == PointType.BOX) {
                                ...
                                if (i != y) {
                                    tmPoints.add(new Point(point.x, point.y + 1, false, PointType.BOX));
                                }
            ...
            for (Point fallingPoint : mFallingPoints) {
                ...
                tmPoints.add(new Point(fallingPoint.x, fallingPoint.y + 1, true, PointType.BOX));
            }
            mFallingPoints.clear();
            ...
            for (int i = 0; i < UPCOMING_AREA_SIZE; i++) {
                for (int j = 0; j < UPCOMING_AREA_SIZE; j++) {
                    if (mUpcomingPoints[i][j].type == PointType.BOX) {
                        mFallingPoints.add(new Point(j + 3, i - 4, true, PointType.BOX));
                    }
                }
            }
            generateUpcomingBrick();
        ...
        switch (turn) {
            case LEFT:
                ...
                if (canTurn) {
                    ...
                    for (Point fallingPoint : mFallingPoints) {
                        tmPoints.add(new Point(fallingPoint.x - 1, fallingPoint.y, true, PointType.BOX));
                        fallingPoint.type = PointType.EMPTY;
                        fallingPoint.isFallingPoint = false;
                    }
                    mFallingPoints.clear();
    ...
    public boolean rotatePoints(int x, int y, int size) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                ...
                points[i][j] = new Point(x + size - 1 - i, y + j, point.isFallingPoint, point.type);
            }
            if (!canRotate) {
                break;
            }
